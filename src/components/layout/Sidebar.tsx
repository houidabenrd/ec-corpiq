...
const navItems: NavItem[] = [
  ...
  },
  {
    ...
    badge: (s) =>
      ...
      s === 'MEMBER_IN_PROGRESS' ? <Badge variant="warning" dot>Renouvellement en cours</Badge> :
...
export function Sidebar({ mobile = false, onClose }: SidebarProps) {
  ...
  return (
    <aside className={clsx(
      ...
      {isExpired && (
        <div className="mx-3 mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
          ...
          <button
            ...
            className="mt-2 w-full text-xs font-semibold text-white bg-red-600 hover:bg-red-700 rounded-md py-1.5 transition-colors"
          >
            Renouveler / Payer
          </button>
        </div>
      ...
      {isNonMember && (
        <div className="mx-3 mt-3 p-3 bg-corpiq-blue-50 border border-corpiq-blue-100 rounded-lg">
          ...
          <button
            onClick={() => handleNav('/adhesion')}
            className="mt-2 w-full text-xs font-semibold text-white bg-corpiq-blue hover:bg-corpiq-blue-light rounded-md py-1.5 transition-colors"
          >
            Adhérer maintenant
          </button>
        </div>
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                isActive && !isGrayed && 'bg-corpiq-blue text-white',
                !isActive && !isGrayed && 'text-gray-700 hover:bg-gray-50',
                isGrayed && 'text-gray-400 cursor-not-allowed opacity-60',
              )}
            >
              <span className={clsx(
                'flex-shrink-0',
                isActive && !isGrayed ? 'text-white' : isGrayed ? 'text-gray-300' : 'text-gray-400 group-hover:text-gray-600'
              )}>
                {item.icon}
              </span>
